...
void checkCctv2(int x, int y, int dir, int tempArr[][9]) {
	while (N > y && M > x && x >= 0 && y >= 0 && tempArr[y][x] != 6) {
		if (tempArr[y][x] == 0) { // 빈 칸일 때만 감시 구역으로 표시
			tempArr[y][x] = 7; // 감시 구역을 구분하기 위해 7로 표시
		}
		x = x + dx[dir];
		y = y + dy[dir];
	}
...
void cctvRot() {
	int tempArr[9][9];
	copy(&arr[0][0], &arr[0][0] + 9 * 9, &tempArr[0][0]);
